"""
This is a depletion input for depletion up to t=20 and then taking the xs from there.
The intent is to reuse the depletion matrix in a follow up calculation (for faster testing.)
where we dont tally the depletion matrix every timestep (and thus have faster solution times as 
compared to full depletion where every single step we tally the depletion matrix).

This input has a single burnable material and only a single unique fuel region.
Thus no density profile is imposed. 
This is OK since in the followup calculations we impose a density profile.
"""
...
water_mats = []
pins = []
fuel_mats.append(UO2Material(temp=600, name=f"fuel_mat"))
...
for idx, fuel in enumerate(water_mats):
  pins.append(
    LWRPincell(name=f'cell_{idx}',
                            ...
                            Water=water_mats[idx], UO2=fuel_mats[0], Zirc=zirc, Helium=helium)
  )
